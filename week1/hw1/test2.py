...
def readDataFile1(fileName):
    ...
    # universal new line, to parse lines by special delimiter
    ...
    for line in fileHeader:
        ...
        # if no missing values and year is either 2000 or 2010
        if isMissing(line) == False:
...
def write_csv(dict_list, fileName, labels):
    # binary write mode, more stable, use 'w'/'r' only for .txt file
    fileHeader = open(fileName, 'wb')
...
def main():
    ...
    indicatorData = readDataFile1('si601_w15_hw1/world_bank_indicators.txt')
    regionHash = readDataFile2('si601_w15_hw1/world_bank_regions.txt')
    ...
    # step 5: 3-level sort
    indicatorData = sorted(
        finalData, key=lambda x: (x['date'], x['region'], x['gdp_per_cap']))
    # step 6: write to .csv file
    labels = ['country', 'date', 'population', 'mobile_sub', 'health', 'internetUser',
              'gdp_per_cap', 'mobile_per_cap', 'log_gdp_per_cap', 'log_health', 'region']

    write_csv(finalData, 'worldbank_output_caogl.csv', labels)
