...
def step1_output(filename):
    ...
    for country in countries:
        ...
        for item in step1_list:
            if country == item[0]:
                totalpopulation += float(item[1].replace('"',
                                                         '').replace(',', ''))
                count1 += 1
                if item[2] == '':
                    continue
                urbanpopulation += float(item[2].replace(',',
                                                         '').replace('"', ''))
                count2 += 1
                if item[3] == '':
                    continue
...
def step2_output(filename):

    step1_output(filename)
...
def step3_output(filename):

    step2_output(filename)
...
def main():
    ...
    step1_output('world_bank_indicators.txt')
    step2_output('world_bank_regions.txt')
    step3_output('world_bank_regions.txt')
