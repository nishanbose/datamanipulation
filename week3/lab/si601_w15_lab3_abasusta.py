...
def process_json(json_str):
  ...
  total = 0
  for k, v in pythondict.iteritems():
    multiply = reduce(lambda x, y: x*y, v)
    total += multiply
  return total
...
def get_pigs(html_doc):
  soup = BeautifulSoup(html_doc)
  pigs = []
  for lines in soup.find("table").find_all("tr")[1:]:
    pig = lines.select("td:nth-of-type(1)")
    pigs.append(str(pig[0].string))
...
def get_story_headings(html_doc):
  soup = BeautifulSoup(html_doc)
  stories = []
  for lines in soup.find_all("h2"):
    if lines.parent.name != "li":
      stories.append(lines.string)
...
def get_houses(html_doc):
  ...
  for lines in soup.find("table").find_all("tr")[1:]:
    ...
    pig = lines.select("td")
    for house in pig:
      houses.append(str(house.string))
    pigs.append(houses)
  return json.dumps(pigs)
...
def treasure_hunting():
  goto = urllib2.urlopen("http://example.com")
  gotosouphtml = goto.read()
  gotosoup = BeautifulSoup(gotosouphtml)
  url = gotosoup.find("a")
  link = url.get("href")

  target = urllib2.urlopen(link)
  targethtml = target.read()
  targetsoup = BeautifulSoup(targethtml)

  table = targetsoup.find("table", class_="iana-table").find("tbody")

  getrow = table.find_all("tr")[2]
  cell = getrow.find_all("td")[0]
...
def find_first_girl(xmlstr):
  xml = ET.fromstring(xmlstr)
  firstfemale = xml.find(".//artist[@sex=\"female\"]")
  return firstfemale.text
...
def find_most_pricy(xmlstr):
  xml = ET.fromstring(xmlstr)
  prices = xml.findall('.//price')
  mostexpensive = 0.0
  for price in prices:
    if mostexpensive < float(price.text):
      mostexpensive = float(price.text)
...
def get_uk_artists(xmlstr):
  ...
  for CD in CDs:
      ...
      if country == 'UK':
        ...
        UKartists.append(artist)
  return sorted(UKartists)
